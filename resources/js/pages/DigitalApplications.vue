  ...
  <Layout>
    <div class="max-w-7xl mx-auto py-8 px-4 space-y-6">
      <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 class="text-2xl md:text-3xl font-bold text-[#005eb8]">ЦОР / МООК ППС</h1>
          <p class="text-sm text-gray-500">Очередь директора Центра искусственного интеллекта и цифрового развития.</p>
        </div>
      ...
      <div v-else-if="applications.length === 0" class="text-center text-gray-600">Заявок ППС для ЦОР / МООК пока нет.</div>

      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <router-link
          v-for="app in applications"
          :key="app.id"
          :to="{ name: 'DigitalApplicationDetails', params: { id: app.id } }"
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <div
              :class="digitalMoocReady(app) ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'"
              class="shrink-0 rounded-full px-2.5 py-1 text-xs font-medium text-center"
            >
              {{ digitalMoocReady(app) ? 'Заполнено' : 'Нужно заполнить' }}
            </div>
          </div>
          ...
          <div class="mt-4 text-sm text-gray-600 line-clamp-3 min-h-[60px]">
            {{ app.pps_profile?.digital_mooc || 'Наличие ЦОР / МООК пока не заполнено.' }}
          </div>

          <div class="mt-3 text-xs text-gray-400">
            Документов: {{ app.pps_profile?.digital_mooc_documents?.length || 0 }}
          </div>
...
const digitalMoocReady = (application) => Boolean(
  application?.pps_profile?.digital_mooc || (application?.pps_profile?.digital_mooc_documents || []).length,
);
...
const fetchApplications = async () => {
  ...
  try {
    const response = await axios.get('/api/digital/applications');
