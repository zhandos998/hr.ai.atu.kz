  ...
  <Layout>
    <div class="max-w-6xl mx-auto py-8 px-4 space-y-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div
          v-if="application"
          :class="digitalMoocReady ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'"
          class="inline-flex px-3 py-1 rounded-full text-sm font-medium"
        >
          {{ digitalMoocReady ? 'ЦОР / МООК заполнено' : 'ЦОР / МООК не заполнено' }}
        </div>
      </div>
        ...
        <div class="space-y-5">
          ...
          <section class="bg-white rounded-2xl shadow border border-gray-100 p-5 space-y-4">
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-[#005eb8]">Наличие ЦОР / МООК</h2>
                <p class="text-sm text-gray-500">Этот блок редактирует директор Центра искусственного интеллекта и цифрового развития.</p>
              </div>
              <button
                :disabled="saving"
                @click="saveDigitalMooc"
              ...
              >
                {{ saving ? 'Сохранение...' : 'Сохранить ЦОР / МООК' }}
              </button>
            </div>

            <label class="space-y-2 block">
              <span class="text-sm font-medium text-gray-700">Описание / наличие ЦОР и МООК</span>
              <textarea
                v-model="digitalMoocDraft"
                rows="8"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white"
                placeholder="Укажите наличие ЦОР, МООК, цифровых курсов, платформ и других цифровых учебных материалов"
              ></textarea>
            </label>
            ...
            <div class="rounded-xl border border-dashed border-gray-300 bg-slate-50 px-4 py-4 space-y-3">
              <div>
                ...
                <div class="text-sm text-gray-500">
                  Можно добавить несколько файлов. Загрузка необязательна.
                </div>
              </div>
              ...
              <input
                ...
                @change="setDigitalMoocFiles"
              />

              <ul v-if="pendingFiles.length" class="space-y-1 text-sm text-gray-600">
                <li v-for="file in pendingFiles" :key="`${file.name}-${file.size}`">
                  {{ file.name }}
                </li>
              </ul>
            </div>
            ...
            <div class="space-y-3">
              ...
              <div v-if="digitalMoocDocuments.length" class="space-y-2">
                <div
                  v-for="document in digitalMoocDocuments"
                  :key="document.id"
                ...
                >
                  ...
                  <button
                    @click="deleteDigitalMoocDocument(document.id)"
              ...
              <div v-else class="text-sm text-gray-500">Документы ЦОР / МООК не прикреплены.</div>
            </div>
          </section>
        </div>
      </div>
...
const digitalMoocReady = computed(() => Boolean(
  application.value?.pps_profile?.digital_mooc || digitalMoocDocuments.value.length,
));

const digitalMoocDocuments = computed(() => application.value?.pps_profile?.digital_mooc_documents || []);
...
const syncDraft = () => {
  digitalMoocDraft.value = application.value?.pps_profile?.digital_mooc || '';
  pendingFiles.value = [];
};
...
const fetchApplication = async () => {
  ...
  try {
    const response = await axios.get(`/api/digital/applications/${route.params.id}`);
...
const setDigitalMoocFiles = (event) => {
  pendingFiles.value = Array.from(event.target.files || []);
};

const saveDigitalMooc = async () => {
  ...
  try {
    ...
    formData.append('digital_mooc', digitalMoocDraft.value || '');

    pendingFiles.value.forEach((file) => {
      formData.append('documents[]', file);
    });

    await axios.post(`/api/digital/applications/${application.value.id}/digital-mooc`, formData, {
      headers: {
    ...
    alert(error?.response?.data?.message || 'Ошибка при сохранении ЦОР / МООК.');
  } finally {
    saving.value = false;
  }
};

const deleteDigitalMoocDocument = async (documentId) => {
  if (!application.value) return;
  if (!confirm('Удалить документ ЦОР / МООК?')) return;

  try {
    await axios.delete(`/api/digital/applications/${application.value.id}/digital-mooc-documents/${documentId}`);
    await fetchApplication();
  } catch (error) {
    alert(error?.response?.data?.message || 'Ошибка при удалении документа ЦОР / МООК.');
  }
};
