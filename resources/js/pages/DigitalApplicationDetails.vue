                ...
                <div class="space-y-5">
                    ...
                    <section class="bg-white rounded-2xl shadow border border-gray-100 p-5 space-y-4">
                        ...
                        <div class="space-y-3">
                            ...
                            <div
                                v-if="digitalMoocDocuments.length"
                                class="space-y-2"
                            >
                                <div
                            ...
                            <div
                                v-else
                                class="text-sm text-gray-500"
                            >Документы ЦОР / МООК не прикреплены.</div>
                        </div>
                    </section>
                </div>
            </div>
...
const digitalMoocReady = computed(() =>
    Boolean(
        application.value?.pps_profile?.digital_mooc ||
            digitalMoocDocuments.value.length
    )
);

const digitalMoocDocuments = computed(
    () => application.value?.pps_profile?.digital_mooc_documents || []
);
...
const fetchApplication = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/digital/applications/${route.params.id}`
        );
...
const saveDigitalMooc = async () => {
    ...
    try {
        ...
        await axios.post(
            `/api/digital/applications/${application.value.id}/digital-mooc`,
            formData,
            {
                headers: {
        ...
        alert(
            error?.response?.data?.message ||
                "Ошибка при сохранении ЦОР / МООК."
        );
    } finally {
        saving.value = false;
    }
...
const deleteDigitalMoocDocument = async (documentId) => {
    ...
    try {
        await axios.delete(
            `/api/digital/applications/${application.value.id}/digital-mooc-documents/${documentId}`
        );
        await fetchApplication();
    } catch (error) {
        alert(
            error?.response?.data?.message ||
                "Ошибка при удалении документа ЦОР / МООК."
        );
    }
};
