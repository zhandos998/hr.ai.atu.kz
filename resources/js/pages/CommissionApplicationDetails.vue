<template>
    <Layout>
        <div class="max-w-6xl mx-auto py-8 px-4 space-y-6">
            ...
            <div
                v-else
                class="grid grid-cols-1 gap-5 xl:grid-cols-[320px,1fr]"
            >
                <div class="space-y-5">
                    <section class="rounded-2xl border border-gray-100 bg-white p-5 shadow space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <div
                                v-if="!application.resume_url && !orderedDocuments.length"
                                class="text-sm text-gray-500"
                            >
                    ...
                    <section class="rounded-2xl border border-gray-100 bg-white p-5 shadow space-y-4">
                        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                            ...
                            <div
                                v-if="application.vote_summary?.my_vote"
                                class="text-sm text-gray-600"
                            >
                                Текущий выбор:
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <div
                                    v-if="field.documentsField && ppsDocuments(field.documentsField).length"
                                    class="flex flex-wrap gap-2"
                                >
                                    <a
                                        v-for="document in ppsDocuments(field.documentsField)"
...
const canVote = computed(() =>
    ["not_started", "voting"].includes(application.value?.hiring_status)
);
...
const ppsSections = [
    {
        ...
        description:
            "Сведения от департамента науки и центра цифрового развития.",
        columnsClass: "md:grid-cols-1",
        fields: [
            {
                key: "scientific_works",
                label: "Научные труды преподавателя",
                documentsField: "scientific_works_documents",
            },
            {
                key: "digital_mooc",
                label: "Наличие ЦОР / МООК",
                documentsField: "digital_mooc_documents",
            },
            {
                key: "science_conclusion",
                label: "Заключение",
            },
        ],
    },
        ...
        fields: [
            {
                ...
                documentsField: "compliance_documents",
            },
            {
...
const fetchApplication = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/commission/applications/${route.params.id}`
        );
...
const submitVote = async (decision, hireTermYears = null) => {
    ...
    try {
        const response = await axios.post(
            `/api/commission/applications/${application.value.id}/vote`,
            {
                decision,
        ...
        alert(
            error?.response?.data?.message || "Ошибка при сохранении голоса."
        );
    } finally {
