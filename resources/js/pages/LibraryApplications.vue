  ...
  <Layout>
    <div class="max-w-7xl mx-auto py-8 px-4 space-y-6">
      <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 class="text-2xl md:text-3xl font-bold text-[#005eb8]">Научная библиотека ППС</h1>
          <p class="text-sm text-gray-500">Очередь директора Научной библиотеки по преподавательским заявкам.</p>
        </div>
      ...
      <div v-else-if="applications.length === 0" class="text-center text-gray-600">Заявок ППС для Научной библиотеки пока нет.</div>

      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <router-link
          v-for="app in applications"
          :key="app.id"
          :to="{ name: 'LibraryApplicationDetails', params: { id: app.id } }"
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <div
              :class="libraryReady(app) ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'"
              class="shrink-0 rounded-full px-2.5 py-1 text-xs font-medium text-center"
            >
              {{ libraryReady(app) ? 'Заполнено' : 'Нужно заполнить' }}
            </div>
          </div>
          ...
          <div class="mt-4 text-sm text-gray-600 line-clamp-4 min-h-[96px]">
            {{ app.pps_profile?.educational_publication_metrics || 'Показатели учебных изданий пока не заполнены.' }}
          </div>
...
const libraryReady = (application) => Boolean(
  application?.pps_profile?.educational_publication_metrics,
);
...
const fetchApplications = async () => {
  ...
  try {
    const response = await axios.get('/api/library/applications');
