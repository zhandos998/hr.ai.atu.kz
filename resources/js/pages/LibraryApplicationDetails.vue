...
const libraryReady = computed(() =>
    Boolean(application.value?.pps_profile?.educational_publication_metrics)
);
...
const syncDraft = () => {
    metricsDraft.value =
        application.value?.pps_profile?.educational_publication_metrics || "";
};
...
const fetchApplication = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/library/applications/${route.params.id}`
        );
...
const saveLibraryMetrics = async () => {
    ...
    try {
        ...
        await axios.post(
            `/api/library/applications/${application.value.id}/library-metrics`,
            formData,
            {
                headers: {
        ...
        alert(
            error?.response?.data?.message ||
                "Ошибка при сохранении показателей библиотеки."
        );
    } finally {
        saving.value = false;
    }
