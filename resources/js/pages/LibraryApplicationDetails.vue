  ...
  <Layout>
    <div class="max-w-6xl mx-auto py-8 px-4 space-y-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div
          v-if="application"
          :class="libraryReady ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'"
          class="inline-flex px-3 py-1 rounded-full text-sm font-medium"
        >
          {{ libraryReady ? 'Блок библиотеки заполнен' : 'Блок библиотеки не заполнен' }}
        </div>
      </div>
        ...
        <div class="space-y-5">
          ...
          <section class="bg-white rounded-2xl shadow border border-gray-100 p-5 space-y-4">
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-[#005eb8]">Показатели учебных изданий</h2>
                <p class="text-sm text-gray-500">Этот блок редактирует директор Научной библиотеки.</p>
              </div>
              <button
                :disabled="saving"
                @click="saveLibraryMetrics"
            ...
            <label class="space-y-2 block">
              <span class="text-sm font-medium text-gray-700">Показатели учебных изданий</span>
              <textarea
                v-model="metricsDraft"
                rows="8"
...
const libraryReady = computed(() => Boolean(application.value?.pps_profile?.educational_publication_metrics));
...
const syncDraft = () => {
  metricsDraft.value = application.value?.pps_profile?.educational_publication_metrics || '';
};
...
const fetchApplication = async () => {
  ...
  try {
    const response = await axios.get(`/api/library/applications/${route.params.id}`);
...
const saveLibraryMetrics = async () => {
  ...
  try {
    ...
    formData.append('educational_publication_metrics', metricsDraft.value || '');

    await axios.post(`/api/library/applications/${application.value.id}/library-metrics`, formData, {
      headers: {
    ...
    alert(error?.response?.data?.message || 'Ошибка при сохранении показателей библиотеки.');
  } finally {
    saving.value = false;
  }
};
