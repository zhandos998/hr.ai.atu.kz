  ...
  <Layout>
    <div class="max-w-7xl mx-auto py-8 px-4 space-y-6">
      <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 class="text-2xl md:text-3xl font-bold text-[#005eb8]">Научные труды ППС</h1>
          <p class="text-sm text-gray-500">Очередь директора Департамента науки по преподавательским заявкам.</p>
        </div>
      ...
      <div v-else-if="applications.length === 0" class="text-center text-gray-600">Заявок ППС для обработки пока нет.</div>

      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <router-link
          v-for="app in applications"
          :key="app.id"
          :to="{ name: 'ScienceApplicationDetails', params: { id: app.id } }"
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <div
              :class="scientificWorksReady(app) ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'"
              class="shrink-0 rounded-full px-2.5 py-1 text-xs font-medium text-center"
            >
              {{ scientificWorksReady(app) ? 'Заполнено' : 'Нужно заполнить' }}
            </div>
          </div>
...
const fetchApplications = async () => {
  ...
  try {
    const response = await axios.get('/api/science/applications');
