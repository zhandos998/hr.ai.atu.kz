            ...
            >
                ...
                <div class="space-y-5">
                    <section
                        ...
                        class="bg-white rounded-2xl shadow border border-gray-100 p-5 space-y-4"
                    >
                        <div>
...
const complianceReady = computed(() =>
    Boolean(
        application.value?.pps_profile?.anti_corruption_survey_results ||
            application.value?.pps_profile?.disciplinary_actions_info
    )
);
const legalCheckReady = computed(() =>
    ["clear", "flagged"].includes(application.value?.compliance_status)
);
const pageReady = computed(() =>
    isPpsApplication.value ? complianceReady.value : legalCheckReady.value
);
const pageReadyLabel = computed(() => {
    if (isPpsApplication.value) {
        return complianceReady.value
            ? "Блок комплаенса заполнен"
            : "Блок комплаенса не заполнен";
    }

    return legalCheckReady.value
        ? "Юридическая проверка завершена"
        : "Юридическая проверка не завершена";
});

const canSetCorruptionStatus = computed(
    () => application.value?.documents_status === "accepted"
);
...
const fetchApplication = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/compliance/applications/${route.params.id}`
        );
...
const setCorruptionStatus = async (statusCode) => {
    ...
    try {
        await axios.put(
            `/api/lawyer/applications/${application.value.id}/corruption-status`,
            {
        ...
        alert(
            error?.response?.data?.message ||
                "Ошибка при сохранении юридической проверки."
        );
    } finally {
...
const saveComplianceDepartment = async () => {
    ...
    try {
        ...
        formData.append(
            "anti_corruption_survey_results",
            complianceDraft.value.anti_corruption_survey_results || ""
        );
        formData.append(
            "disciplinary_actions_info",
            complianceDraft.value.disciplinary_actions_info || ""
        );

        await axios.post(
            `/api/compliance/applications/${application.value.id}/department-review`,
            formData,
            {
                headers: {
        ...
        alert(
            error?.response?.data?.message ||
                "Ошибка при сохранении данных комплаенса."
        );
    } finally {
        saving.value = false;
    }
...
const openLawyerResponsePdf = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/compliance/applications/${application.value.id}/lawyer-response-pdf`,
            {
                responseType: "blob",
            }
        );
