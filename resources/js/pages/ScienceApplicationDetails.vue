                        ...
                        <div class="space-y-3">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                >
                                    ...
                                    <div
                                        v-if="ppsDocuments(field.documentsField).length"
                                        class="flex flex-wrap gap-2"
                                    >
                                        <a
                    ...
                    <section class="bg-white rounded-2xl shadow border border-gray-100 p-5 space-y-4">
                        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                            ...
                            <div class="flex flex-col gap-2 sm:flex-row">
                                <button
                                    :disabled="!canGenerateSciencePdf || pdfLoading || saving"
                                    @click="openScienceResponsePdf"
                        ...
                        <label class="space-y-2 block">
                            ...
                            <textarea
                                v-model="scienceConclusionDraft"
                                rows="6"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white"
                        ...
                        <div class="space-y-3">
                            ...
                            <div
                                v-if="scientificWorksDocuments.length"
                                class="space-y-2"
                            >
                                <div
                            ...
                            <div
                                v-else
                                class="text-sm text-gray-500"
                            >Документы по научным трудам не прикреплены.</div>
                        </div>
                    </section>
                </div>
            </div>
...
const scienceConclusionDraft = ref("");
...
const scientificWorksDocuments = computed(
    () => application.value?.pps_profile?.scientific_works_documents || []
);

const scientificWorksReady = computed(() =>
    Boolean(
        application.value?.pps_profile?.scientific_works ||
            application.value?.pps_profile?.science_conclusion ||
            scientificWorksDocuments.value.length
    )
);

const canGenerateSciencePdf = computed(() =>
    Boolean(scienceConclusionDraft.value?.trim())
);
...
const syncDraft = () => {
    scientificWorksDraft.value =
        application.value?.pps_profile?.scientific_works || "";
    scienceConclusionDraft.value =
        application.value?.pps_profile?.science_conclusion || "";
    pendingFiles.value = [];
};
...
const fetchApplication = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/science/applications/${route.params.id}`
        );
...
const saveScientificWorks = async () => {
    ...
    try {
        ...
        formData.append("science_conclusion", scienceConclusionDraft.value || "");
        ...
        await axios.post(
            `/api/science/applications/${application.value.id}/scientific-works`,
            formData,
            {
                headers: {
        ...
        alert(
            error?.response?.data?.message ||
                "Ошибка при сохранении научных трудов."
        );
    } finally {
        saving.value = false;
    }
...
const openScienceResponsePdf = async () => {
    if (!application.value || !canGenerateSciencePdf.value) {
    ...
    try {
        ...
        const response = await axios.get(
            `/api/science/applications/${application.value.id}/science-response-pdf`,
            {
                responseType: "blob",
            }
        );
...
const deleteScientificWorksDocument = async (documentId) => {
    ...
    try {
        await axios.delete(
            `/api/science/applications/${application.value.id}/scientific-works-documents/${documentId}`
        );
        await fetchApplication();
    } catch (error) {
        alert(
            error?.response?.data?.message ||
                "Ошибка при удалении документа научных трудов."
        );
    }
};
