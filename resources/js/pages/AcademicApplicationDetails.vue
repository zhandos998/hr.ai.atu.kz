                ...
                <div class="space-y-5">
                    ...
                    <section class="bg-white rounded-2xl shadow border border-gray-100 p-5 space-y-4">
                        ...
                        <div class="flex justify-end">
                            <button
                                :disabled="!canGenerateAcademicPdf || pdfLoading || saving"
                                @click="openAcademicResponsePdf"
                                class="inline-flex items-center justify-center bg-[#005eb8] hover:bg-blue-700 disabled:bg-gray-300 text-white text-sm font-semibold px-4 py-2 rounded-lg transition"
                            >
                                {{ pdfLoading ? 'Генерация PDF...' : 'Сгенерировать' }}
                            </button>
                        </div>
                    </section>
                </div>
...
const canGenerateAcademicPdf = computed(() =>
    Boolean(academicDraft.value.academic_conclusion?.trim())
);
...
const fetchApplication = async () => {
    ...
    try {
        const response = await axios.get(
            `/api/academic/applications/${route.params.id}`
        );
...
const persistAcademicReview = async () => {
    if (!application.value) return;

    const formData = new FormData();
    formData.append(
        "open_lesson_quality",
        academicDraft.value.open_lesson_quality || ""
    );
    formData.append(
        "taught_disciplines",
        academicDraft.value.taught_disciplines || ""
    );
    formData.append(
        "educational_methodical_literature",
    ...
    formData.append(
        "academic_conclusion",
        academicDraft.value.academic_conclusion || ""
    );

    await axios.post(
        `/api/academic/applications/${application.value.id}/academic-review`,
        formData,
        {
            headers: {
...
const saveAcademicReview = async () => {
    ...
    try {
        await persistAcademicReview();
    } catch (error) {
        alert(
            error?.response?.data?.message ||
                "Ошибка при сохранении данных академического развития."
        );
    } finally {
        saving.value = false;
    }
...
const openAcademicResponsePdf = async () => {
    if (!application.value || !canGenerateAcademicPdf.value) {
    ...
    try {
        ...
        const response = await axios.get(
            `/api/academic/applications/${application.value.id}/academic-response-pdf`,
            {
                responseType: "blob",
            }
        );
