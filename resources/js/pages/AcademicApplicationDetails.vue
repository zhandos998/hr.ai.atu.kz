  ...
  <Layout>
    <div class="max-w-6xl mx-auto py-8 px-4 space-y-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div
          v-if="application"
          :class="academicReady ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'"
          class="inline-flex px-3 py-1 rounded-full text-sm font-medium"
        >
          {{ academicReady ? 'Академический блок заполнен' : 'Академический блок не заполнен' }}
        </div>
      </div>
        ...
        <div class="space-y-5">
          ...
          <section class="bg-white rounded-2xl shadow border border-gray-100 p-5 space-y-4">
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-[#005eb8]">Показатели академического развития</h2>
                <p class="text-sm text-gray-500">Этот блок редактирует директор Департамента академического развития.</p>
              </div>
              <button
                :disabled="saving"
                @click="saveAcademicReview"
            ...
            <label class="space-y-2 block">
              <span class="text-sm font-medium text-gray-700">Оценка качества проведения открытого занятия</span>
              <textarea
                v-model="academicDraft.open_lesson_quality"
                rows="5"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white"
                placeholder="Укажите оценку качества проведения открытого занятия"
              ></textarea>
            </label>

            <label class="space-y-2 block">
              <span class="text-sm font-medium text-gray-700">Преподаваемые дисциплины</span>
              <textarea
                v-model="academicDraft.taught_disciplines"
                rows="5"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white"
                placeholder="Перечислите преподаваемые дисциплины"
              ></textarea>
            </label>

            <label class="space-y-2 block">
              <span class="text-sm font-medium text-gray-700">Учебно-методическая литература</span>
              <textarea
                v-model="academicDraft.educational_methodical_literature"
                rows="5"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white"
                placeholder="Укажите сведения по учебно-методической литературе"
              ></textarea>
            </label>
          </section>
        </div>
...
const academicReady = computed(() => Boolean(
...
const syncDraft = () => {
  academicDraft.value = {
    open_lesson_quality: application.value?.pps_profile?.open_lesson_quality || '',
    taught_disciplines: application.value?.pps_profile?.taught_disciplines || '',
    educational_methodical_literature: application.value?.pps_profile?.educational_methodical_literature || '',
  };
};
...
const fetchApplication = async () => {
  ...
  try {
    const response = await axios.get(`/api/academic/applications/${route.params.id}`);
...
const saveAcademicReview = async () => {
  ...
  try {
    ...
    formData.append('open_lesson_quality', academicDraft.value.open_lesson_quality || '');
    formData.append('taught_disciplines', academicDraft.value.taught_disciplines || '');
    formData.append('educational_methodical_literature', academicDraft.value.educational_methodical_literature || '');

    await axios.post(`/api/academic/applications/${application.value.id}/academic-review`, formData, {
      headers: {
    ...
    alert(error?.response?.data?.message || 'Ошибка при сохранении данных академического развития.');
  } finally {
    saving.value = false;
  }
};
