        ...
        <div class="space-y-5">
          ...
          <section class="bg-white rounded-2xl shadow border border-gray-100 p-5 space-y-4">
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-[#005eb8]">Показатели стратегического развития</h2>
...
const strategyReady = computed(() => Boolean(
...
const syncDraft = () => {
  strategyDraft.value = {
    final_rating_score: application.value?.pps_profile?.final_rating_score || '',
    student_survey_results: application.value?.pps_profile?.student_survey_results || '',
    individual_plan_nonfulfillment: application.value?.pps_profile?.individual_plan_nonfulfillment || '',
...
const fetchApplication = async () => {
  ...
  try {
    const response = await axios.get(`/api/strategy/applications/${route.params.id}`);
...
const saveStrategyReview = async () => {
  ...
  try {
    ...
    formData.append('final_rating_score', strategyDraft.value.final_rating_score || '');
    formData.append('student_survey_results', strategyDraft.value.student_survey_results || '');
    formData.append('individual_plan_nonfulfillment', strategyDraft.value.individual_plan_nonfulfillment || '');
    formData.append('krk', strategyDraft.value.krk || '');

    await axios.post(`/api/strategy/applications/${application.value.id}/strategy-review`, formData, {
      headers: {
    ...
    alert(error?.response?.data?.message || 'Ошибка при сохранении стратегических показателей.');
  } finally {
    saving.value = false;
  }
};
